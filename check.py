...
class Layout:
    ...
    def CheckRepoDown(self):
        ...
        merger_config_file = '/install/galaxy/etc/config/merger/config.json'
        hostfile='/etc/hostname'
        downCount = 0
        try:
            if os.path.exists(merger_config_file) and os.path.exists(hostfile):
                try:
                    op1 = subprocess.check_output(['grep', '-c', '"alive": false', '/install/galaxy/etc/config/merger/config.json']).decode('ascii').strip()
                    downCount = int(op1)
    ...
    def GetIngestionEPS(self):
        NORM_FRONT_PATH = "/install/galaxy/var/log/benchmarker/norm_front.log"
        STORE_HANDLER_PATH = "/install/galaxy/var/log/benchmarker/store_handler.log"
        BUCKET = 10 * 60
